...
2. **Resource services** (`s1`, `s2`, …) that verify those tokens and authorize
   requests.
...
- **Access tokens** are short-lived JWTs (default 15 min). **Refresh tokens** are
  opaque, single-use, and rotate on every refresh.
...
Continue with the OTP step (§1.3) using the `mfa_token`.

**Failure** → `400` `{ "error": "invalid_grant", "error_description": "Invalid credentials" }`.
...
### 1.4 Sign in with Google (redirect-only)

1. Send the browser to:
   `GET /auth/google/initiate?client_id=s1&redirect_uri=https://s1.domain.com/callback`
   (the `redirect_uri` must be registered on the application).
2. Darboon redirects to Google, then back to your `redirect_uri` with tokens in
   the **URL fragment**:
   `https://s1.domain.com/callback#access_token=…&refresh_token=…&token_type=Bearer&expires_in=900`
3. Read the fragment client-side and store the tokens (see §1.6).
...
treated as theft and **revokes the entire token family** (you'll get
`invalid_grant` and the user must log in again).

### 1.6 Token storage guidance

- Prefer **in-memory** access tokens; keep refresh tokens in secure,
  `HttpOnly`+`Secure`+`SameSite` cookies set by your dashboard's own backend.
- If you must store on the client, treat tokens as secrets — never log them, and
  scrub them from URLs after reading the Google callback fragment.
...
| Confirm email | `GET /verify/email/confirm?token=…` (link) or `POST /verify/email/confirm { token }` |
...
@Module({
  imports: [DarboonAuthModule.forRoot({ issuer: 'https://auth.domain.com', audience: 's1' })],
...
`@Public()`, and read the principal with `@CurrentUser()`. See the SDK
[README](../packages/nestjs-verifier/README.md).
...
For high-security endpoints, additionally call introspection (requires the
resource-server API key):
...
~30s). Otherwise rely on short access-token TTLs.
...
the updated `roles` claim (the RBAC cache TTL is ~60s).
